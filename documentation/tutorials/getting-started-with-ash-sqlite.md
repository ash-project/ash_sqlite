...
AshSqlite supports loading, filtering, sorting, and expression calculations for related `count`, `sum`, `avg`, `min`, `max`, and `exists` aggregates over a single normal relationship. Aggregate filters and aggregate `join_filter`s are supported when they do not depend on parent row values.
