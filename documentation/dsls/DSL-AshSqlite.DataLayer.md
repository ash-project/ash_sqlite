...
| [`tenant_binder`](#sqlite-tenant_binder){: #sqlite-tenant_binder } | `module` |  | A module that selects the connection a tenanted statement runs on. Required for `strategy :context`, where each tenant has its own database file. See `AshSqlite.TenantBinder`. |
